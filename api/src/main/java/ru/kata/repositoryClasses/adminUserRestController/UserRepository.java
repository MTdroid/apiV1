...
/*    public static List<DeleteUserResponse> deleteUser(Integer id) {
    ...
    public static void deleteReq(Integer id) {
        final String hql = "DELETE FROM recruitment WHERE id =:id" ;

        Transaction tr = session.beginTransaction();
        session.createNativeQuery(hql, UserTable.class).setParameter("id",id).executeUpdate();
        tr.commit();

    }    public static void deleteReq1(Integer id) {
        final String hql = "DELETE FROM recruitment_meet WHERE id =:id" ;

        Transaction tr = session.beginTransaction();
        session.createNativeQuery(hql, UserTable.class).setParameter("id",id).executeUpdate();
        tr.commit();

    }        public static void deleteReq2(Integer id) {
        final String hql = "DELETE FROM recruitment_meet_entry WHERE id =:id" ;
